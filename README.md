...
## Two data bugs caught and fixed during analysis (not just assumed correct)

**Hidden agent-count mismatch:** grouping tickets by agent returned 21 rows instead of the expected 20. Traced to the `'Unknown'` placeholder agent legitimately being counted as its own group - confirmed correct behavior, then excluded it specifically from the per-agent performance KPI so it isn't ranked against real agents (`sql/02_kpi_queries.sql`, KPI 3).

**Silent line-ending bug:** a reopened-ticket-rate query returned 0 reopened tickets despite `GROUP BY status` clearly showing 239 rows with status `'Reopened'`. Root cause: the source CSV uses Windows-style `\r\n` line endings, but the original `LOAD DATA` script only declared `\n` as the line terminator - leaving a stray, invisible carriage return glued onto every value in the last column (`status`). `GROUP BY`/`COUNT` still worked (they just bucket identical values, whatever they are), but any exact match against `'Reopened'` silently matched nothing. Fixed both the already-loaded data (`UPDATE ... REPLACE(status, '\r', '')`) and the load script itself (`LINES TERMINATED BY '\r\n'`) so it won't recur on a fresh load.

## Key findings

- **Priority-based triage is working as intended:** Urgent tickets resolve in **1.85 hours** on average versus **6.36 hours** for Low priority - roughly 3.4x faster, confirming the prioritization logic actually affects outcomes rather than being cosmetic.
- **Technical Issue is the leading driver of ticket volume** (1,468 tickets, ~29%), followed by Billing (993) and Product Question (794).
- **Email is the dominant support channel** (2,286 tickets, ~46%), more than Phone and Chat combined.
- **Customer satisfaction is stable across categories** (3.72-3.81 out of 5) - no single category stands out as a major pain point, though survey response rates hover around 64-67%, meaning roughly a third of tickets have no satisfaction signal at all.
- **4.78% of tickets get reopened** (239 of 5,000) - a rework/quality baseline worth tracking over time once more data is available.
...
- **Excel** and **Power BI** - visual dashboards (in progress, see `excel/` and `powerbi/`)
...
excel/        Excel dashboard (in progress)
powerbi/      Power BI report (in progress)
```
